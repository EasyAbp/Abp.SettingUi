...
WORKDIR /templates/service/host/EasyAbp.Abp.SettingUi.Host
RUN dotnet restore
RUN dotnet ef migrations script -i -o migrations-SettingUi.sql
...
COPY --from=build /templates/service/host/EasyAbp.Abp.SettingUi.Host/migrations-SettingUi.sql migrations-SettingUi.sql
